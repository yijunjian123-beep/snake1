...
export const DEFAULT_PVP_SERVER_CONFIG: PvpServerConfig = {
  port: 8787,
  wsPath: "/ws",
  maxConnections: 250,
  maxRooms: 120,
...
function parseAllowedOrigins(value: string | undefined, env: NodeJS.ProcessEnv): readonly string[] {
  if (value === undefined || value.trim() === "") {
...
export function readPvpServerConfig(env: NodeJS.ProcessEnv = process.env): PvpServerConfig {
  return {
    ...
    heartbeatIntervalMs: readPositiveInteger(
      env.PVP_HEARTBEAT_INTERVAL_MS,
      DEFAULT_PVP_SERVER_CONFIG.heartbeatIntervalMs,
    ),
    connectionTimeoutMs: readPositiveInteger(
      env.PVP_CONNECTION_TIMEOUT_MS,
      DEFAULT_PVP_SERVER_CONFIG.connectionTimeoutMs,
    ),
    reconnectGraceMs: readPositiveInteger(
      env.PVP_RECONNECT_GRACE_MS,
      DEFAULT_PVP_SERVER_CONFIG.reconnectGraceMs,
    ),
    ...
    inputDelayTicks: readPositiveInteger(
      env.PVP_INPUT_DELAY_TICKS,
      DEFAULT_PVP_SERVER_CONFIG.inputDelayTicks,
    ),
    messageRateLimitWindowMs: readPositiveInteger(
      env.PVP_MESSAGE_RATE_LIMIT_WINDOW_MS,
      DEFAULT_PVP_SERVER_CONFIG.messageRateLimitWindowMs,
    ),
    messageRateLimitMaxCount: readPositiveInteger(
      env.PVP_MESSAGE_RATE_LIMIT_MAX_COUNT,
      DEFAULT_PVP_SERVER_CONFIG.messageRateLimitMaxCount,
    ),
    cleanupIntervalMs: readPositiveInteger(
      env.PVP_CLEANUP_INTERVAL_MS,
      DEFAULT_PVP_SERVER_CONFIG.cleanupIntervalMs,
    ),
    finishedRoomTtlMs: readPositiveInteger(
      env.PVP_FINISHED_ROOM_TTL_MS,
      DEFAULT_PVP_SERVER_CONFIG.finishedRoomTtlMs,
    ),
    privateRoomWaitTimeoutMs: readPositiveInteger(
      env.PVP_PRIVATE_ROOM_WAIT_TIMEOUT_MS,
      DEFAULT_PVP_SERVER_CONFIG.privateRoomWaitTimeoutMs,
    ),
