...
export function findSafeSpawnPosition(grid, config = {}) {
...
export function findStage0SafeSpawnPosition(grid, config = {}) {
    const occupied = new Set();
    const occupiedCells = [...(config.occupiedCells ?? []), ...(config.blockedCells ?? [])];
    occupiedCells.forEach((cell) => occupied.add(cellKey(cell)));
    const head = config.snakeHead ?? null;
...
export function findReviveSpawnPlacement(grid, config) {
    ...
    headCandidates.sort((left, right) => (left.radius - right.radius ||
    ...
    for (const headCandidate of headCandidates) {
        ...
        const directionCandidates = CARDINAL_DIRECTIONS
            .map((direction) => {
            const forwardCells = getReviveForwardCells(head, direction, grid, blocked, dangerZones, wallPadding);
            if (!forwardCells) {
                return null;
            }
            ...
            .filter((candidate) => candidate !== null && Number.isFinite(candidate.score))
        ...
        for (const directionCandidate of directionCandidates) {
            ...
            const body = [];
            if (buildReviveBodyPath(grid, head, body, visited, length - 1, dangerZones, wallPadding, front, random)) {
...
function pickCandidate(grid, occupied, head, headRadius, wallPadding, dangerZones, rule, maxAttempts, random) {
    const candidates = [];
    for (let row = 0; row < grid.rows; row += 1) {
        for (let col = 0; col < grid.columns; col += 1) {
            ...
            if (occupied.has(cellKey(cell)))
                continue;
            if (!rule.ignoreWallPadding && isWallDanger(cell, grid, wallPadding))
                continue;
            if (!rule.ignoreHeadRadius && head && isNear(cell, head, headRadius))
                continue;
            if (!rule.ignoreDangerZones && dangerZones.some((zone) => isInDangerZone(cell, zone)))
                continue;
...
function isWallDanger(cell, grid, padding) {
    if (padding <= 0)
        return false;
...
function isNear(cell, center, radius) {
    return isWithinChebyshevRadius(cell, center, radius);
}
function isInDangerZone(cell, zone) {
    return isCellInsideZone(cell, zone);
}
function buildReviveBodyPath(grid, current, path, visited, remainingSegments, dangerZones, wallPadding, frontCell, random) {
    ...
    for (const candidate of candidates) {
        ...
        if (buildReviveBodyPath(grid, candidate.cell, path, visited, remainingSegments - 1, dangerZones, wallPadding, frontCell, random)) {
            return true;
        }
        path.pop();
...
function isPlacementCellSafe(cell, grid, blocked, dangerZones, wallPadding) {
...
function scoreReviveHeadCandidate(cell, grid, blocked, dangerZones, wallPadding) {
    const centerColumn = (grid.columns - 1) / 2;
    const centerRow = (grid.rows - 1) / 2;
...
function scoreReviveDirection(direction, head, grid, blocked, dangerZones, wallPadding) {
    const front = stepCell(head, direction);
    if (!isPlacementCellSafe(front, grid, blocked, dangerZones, wallPadding)) {
        return Number.NEGATIVE_INFINITY;
    }
    const scoutBlocked = new Set(blocked);
    scoutBlocked.add(cellKey(head));
    const wallDistance = Math.min(front.column, front.row, grid.columns - 1 - front.column, grid.rows - 1 - front.row);
    ...
    const headWallDistance = Math.min(head.column, head.row, grid.columns - 1 - head.column, grid.rows - 1 - head.row);
    score += headWallDistance * 0.5;
    return score;
}
function getReviveForwardCells(head, direction, grid, blocked, dangerZones, wallPadding) {
    const cells = [];
    let current = head;
...
function scoreRevivePathCell(cell, grid, visited, dangerZones, wallPadding, frontCell) {
    const wallDistance = Math.min(cell.column, cell.row, grid.columns - 1 - cell.column, grid.rows - 1 - cell.row);
...
function getOpenNeighborCount(cell, grid, blocked, dangerZones, wallPadding) {
    let count = 0;
...
function pickFrom(candidates, random) {
    if (candidates.length === 0)
        return null;
    const index = Math.floor(random() * candidates.length);
