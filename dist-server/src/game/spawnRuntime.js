import { createFoodSpawnContext } from "./foodSpawn.js";
...
export function buildFoodSpawnContext(input) {
...
export function getReviveBlockedCells(input) {
    return [
        ...input.snake,
        ...input.foods,
...
export function isCurrentPlacementValid(input) {
    const snakeIsValid = input.snake.every((segment) => !isOutOfBounds(segment, input.grid));
    const occupiedBlackHoleCells = new Set();
    const occupiedStarBeastCells = new Set();
    const occupiedStarCoreCells = new Set();
...
function getStarCoreCell(core) {
    return {
        column: Math.floor(core.x),
        row: Math.floor(core.y),
    };
}
function isOutOfBounds(cell, grid) {
