...
export const PVP_LOBBY_FLOWS = ["matchmaking", "invite", "join"];
...
export function createInitialPvpConnectionState(flow = "matchmaking", joinCode = "") {
    return {
        status: "disconnected",
        flow,
        playerId: null,
...
export function getPvpPanelState(state, now = Date.now()) {
...
function getPlayerLabel(player, currentSlot) {
    return player.playerSlot === currentSlot ? "你" : player.playerSlot.toUpperCase();
}
function getPlayerPresenceLabel(player) {
    if (!player.connected) {
        return "已断开";
    }
