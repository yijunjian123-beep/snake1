...
import { createPvpMessageRates, createPvpMetricsSnapshot, recordPvpIncomingMessage, samplePvpMessageRates, } from "./metrics.js";
import { RoomManager } from "./roomManager.js";
import { countConnectedRoomPlayers, findRoomPlayerById, toProtocolRoomPlayers } from "./roomTypes.js";
import { advancePvpTick, createPvpBoardGrid, createPvpRuntime, createPvpSnapshot, recordPvpInput, } from "../src/pvp/shared/pvpGame.js";
...
function handleHttpRequest(request, response, startedAt, context, shuttingDown) {
    const requestUrl = new URL(request.url ?? "/", "http://localhost");
    if (shuttingDown) {
...
function handleSocketMessage(connection, data, context) {
...
function handleHello(connection, message, context) {
    if (!context.connections.isCurrent(connection) || connection.socket === null) {
        return;
    }
...
function handleMatchmakingJoin(connection, context) {
    const now = Date.now();
...
function handleMatchmakingCancel(connection, context) {
...
function handleCreatePrivateRoom(connection, context) {
    if (connection.status === "in_queue") {
...
function handleJoinPrivateRoom(connection, roomCode, context) {
    if (connection.status === "in_queue") {
...
function handleLeaveRoom(connection, context) {
...
function handleReadyChange(connection, ready, context) {
    if (connection.roomId === null) {
        sendError(context, connection, "not_in_room", "You are not currently in a room");
        return;
    }
...
function handleInputMessage(connection, message, context) {
    const room = context.rooms.getByPlayerId(connection.playerId);
    ...
    const now = Date.now();
    const inputAccepted = recordPvpInput(room.gameState.inputState, player.playerSlot, message.seq, message.tick, message.direction, room.inputDelayTicks, now);
    if (!inputAccepted) {
        return;
    }
...
function startRoomCountdown(roomId, context) {
...
function handleRoomDeparture(connection, context, disconnect) {
...
function handleDisconnectedRoomDeparture(connection, context) {
    const roomId = connection.roomId;
    if (roomId === null) {
        setConnectionDisconnected(connection);
        return;
...
function handleConnectionGone(connection, context) {
...
function sendReconnectResult(connection, context, result) {
...
function restoreSessionRoomState(connection, context) {
...
function drainMatchmakingQueue(context, now = Date.now()) {
...
function broadcastQueueStates(context, now = Date.now()) {
    for (const entry of context.matchmakingQueue.values()) {
        const connection = context.connections.get(entry.playerId);
        if (connection === undefined || connection.status !== "in_queue") {
            context.matchmakingQueue.remove(entry.playerId);
...
function sendQueueState(connection, context, now = Date.now()) {
...
function broadcastRoomState(room, context) {
...
function broadcastToRoom(room, context, message) {
    for (const player of room.players) {
...
function broadcastRoomSnapshot(room, context, snapshot) {
    room.lastSnapshot = snapshot;
    broadcastToRoom(room, context, snapshot);
}
function startRoomGameLoop(room, context) {
...
function advancePlayingRoom(roomId, context) {
    const room = context.rooms.get(roomId);
    if (room === undefined || room.phase !== "playing" || room.gameState === undefined) {
        return;
    }
...
function finalizePlayingRoom(roomId, context, gameOver) {
...
function sweepHeartbeat(context) {
    const now = Date.now();
...
function sweepExpiredRooms(context) {
    const now = Date.now();
...
function sweepPlayingReconnectTimeouts(context, now) {
...
function createWelcomeMessage(connection) {
    return {
        type: "welcome",
        playerId: connection.playerId,
...
function sendError(context, connection, code, message) {
    const room = connection.roomId === null ? undefined : context.rooms.get(connection.roomId);
    logServerEvent("error", {
        errorCode: code,
        playerId: connection.playerId,
...
function setConnectionQueued(connection, joinedAt) {
...
function setConnectionRoomState(connection, room, status) {
    const roomPlayer = findRoomPlayerById(room.players, connection.playerId);
...
function setConnectionIdle(connection) {
...
function setConnectionDisconnected(connection) {
    connection.status = "disconnected";
    connection.queueJoinedAt = undefined;
}
function sendTrackedMessage(context, socket, message) {
    void context;
...
function rejectUpgrade(socket, statusCode, errorCode, message) {
